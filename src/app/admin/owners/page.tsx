...
import { 
  Search, Building2, Edit2, Trash2, UserX, UserCheck, 
  Mail, User, Loader2, AlertTriangle, BadgeCheck
} from 'lucide-react'
...
export default function AdminOwnersPage() {
  ...
  const fetchOwners = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const ownerProfiles = allProfiles.filter((p: any) => p.role === 'owner')
        setOwners(ownerProfiles)
      } else {
        setErrorMsg(result.error || 'Failed to load owners')
      }
    } catch (err: any) {
      setErrorMsg(err.message || 'An error occurred fetching owners')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleEditOwner = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setErrorMsg(result.error || 'Failed to update owner')
      }
    } catch (err: any) {
      setErrorMsg(err.message || 'An error occurred updating owner')
    } finally {
  ...
  const handleDeleteOwner = async () => {
    ...
    setErrorMsg('')
    try {
      const res = await fetch(`/api/admin/users?id=${selectedOwner.id}`, {
        method: 'DELETE'
      })
      const result = await res.json()
      if (result.success) {
        ...
        setErrorMsg(result.error || 'Failed to delete owner')
      }
    } catch (err: any) {
      setErrorMsg(err.message || 'An error occurred deleting owner')
    } finally {
      setActionLoading(false)
    }
  }
  ...
  const toggleOwnerStatus = async (owner: any) => {
    try {
      ...
      alert('An error occurred updating status: ' + err.message)
    }
  }
  ...
  const openEdit = (owner: any) => {
    ...
    setFormPassword('')
    setFormFullName(owner.full_name || '')
    setFormIsActive(owner.is_active !== false)
    setEditOpen(true)
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-black text-foreground">Turf Owners</h1>
        <p className="text-muted-foreground mt-1">Manage turf owner details, verification status, and listing authorization.</p>
      </div>
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Search owners..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      <Card className="glass-dark border-white/10 shadow-xl">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2 font-bold text-foreground">
            <Building2 className="w-5 h-5 text-primary" /> Turf Owners ({filtered.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-muted-foreground">
                    {['Owner Details', 'Email', 'Status', 'Joined', 'Actions'].map(h => (
                ...
                <tbody>
                  {filtered.map((owner, i) => (
                    <motion.tr key={owner.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                      ...
                      <td className="py-3 px-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-semibold text-foreground block">{owner.full_name || 'Unknown'}</span>
                            <span className="text-[10px] text-muted-foreground font-mono block">{owner.id}</span>
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-2 text-muted-foreground">{owner.email || '—'}</td>
                      <td className="py-3 px-2">
                      ...
                      <td className="py-3 px-2 text-muted-foreground">{new Date(owner.created_at).toLocaleDateString('en-IN')}</td>
                      <td className="py-3 px-2">
                        <div className="flex items-center gap-1">
                          <Button size="sm" variant="ghost" onClick={() => openEdit(owner)}
                            className="text-xs h-8 w-8 p-0 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-lg">
                            <Edit2 className="w-3.5 h-3.5" />
                          </Button>

                          <Button size="sm" variant="ghost" onClick={() => toggleOwnerStatus(owner)}
                          ...
                          <Button size="sm" variant="ghost" onClick={() => openDelete(owner)}
              ...
              {filtered.length === 0 && (
                <div className="text-center py-12 text-muted-foreground">No turf owners found.</div>
              )}
            </div>
          )}
        </CardContent>
        ...
        <DialogContent className="glass-dark border-white/10 max-w-md text-foreground rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl font-bold">
              <Edit2 className="w-5 h-5 text-primary" /> Modify Owner Account
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleEditOwner} className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="edit-name" value={formFullName} onChange={(e) => setFormFullName(e.target.value)}
                  placeholder="John Owner" className="pl-10 bg-white/5 border-white/10 rounded-xl" required />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="edit-password" className="text-xs">Update Password (Leave blank to keep same)</Label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        ...
        <DialogContent className="glass-dark border-white/10 max-w-sm text-foreground rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl font-bold text-destructive">
              <Trash2 className="w-5 h-5" /> Delete Owner Account
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground leading-relaxed">
              Are you sure you want to delete owner <span className="text-white font-semibold">{selectedOwner?.full_name}</span>? 
              <br /><br />
              <strong className="text-red-400">WARNING:</strong> This action will cascade-delete their profile, listed turfs, bookings, and payments. This action is irreversible.
            </p>
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setDeleteOpen(false)} className="flex-1 border-white/10 rounded-xl">Cancel</Button>
              <Button onClick={handleDeleteOwner} disabled={actionLoading} className="flex-1 bg-destructive hover:bg-destructive/90 text-white font-bold rounded-xl">
                {actionLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Deleting...</> : 'Delete Owner'}
              </Button>
            </div>
          </div>
        </DialogContent>
